...
metadata {
	definition (name: "Petcurfew Handler", namespace: "", author: "tareker", mnmn: "SmartThings", vid: "generic-switch") {
    //definition (name: "Petcurfew Cat Door Handler", namespace: "", author: "tareker", vid: "generic-contact (contact, battery)") {
	...
	tiles (scale: 2) {
		...
		standardTile("open", "device.door", inactiveLabel: false, decoration: "flat", width: 3, height: 3) {
			state "default", label:'open', action:"door control.open", icon:"st.doors.garage.garage-open"
		}
		standardTile("close", "device.door", inactiveLabel: false, decoration: "flat", width: 3, height: 3) {
			state "default", label:'close', action:"door control.close", icon:"st.doors.garage.garage-closed"
		}
...
{
	httpGet(
		uri: "https://api.particle.io/v1/devices/${deviceId}/doorIsOpen",
